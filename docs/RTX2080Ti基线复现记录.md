...
## 2026-09-13：LPI 五模型 Batch 容量校准

保持固定 640、FP32、无增强，已完成真实 LPI 训练图片容量探针及五模型原生训练/验证链路复核。本机 `nvidia-smi` 总显存 22,528 MiB，选定 RepViT Batch16、MobileNetV4 Batch24、YOLO26n Batch72、YOLO11n Batch80、YOLOv8n Batch96；容量长测整卡采样峰值为 20,664/20,598/19,959/19,415/20,268 MiB。更大候选存在 OOM 或余量不足，因此不采用。显存利用率约 86%～92%，不是完整 200 epoch 的峰值保证。

两个 DiffusionDet 的学习率和迭代调度已按 Batch 同步换算，仍为 200 epoch；YOLO 保持原生优化器配方。新配置首次正式启动前校准，保留原 planned ID，不改历史 SAR 运行。用户要求先训练验证再提交，因此 LPI 入口默认允许未提交工作区，保存源码快照；严格 Git 检查改为可选 `--require-clean-git`。GPU 预检要求可用显存至少 20 GiB。

完整候选、峰值、测试 ID 与一键命令见 [实验日志第 6.10 节](实验日志.md) 和第 5.8 节。五个原生测试均完成，100 项单元测试通过；本轮未启动正式 200 epoch 训练。
